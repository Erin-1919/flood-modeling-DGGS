#==============================================================================
# Flood Modeling using VSURF Random Forest - Resolution 19
#==============================================================================
#
...
# Read sample points
data_file <- paste0("../Data/NB_sample_cent_quanti4_", RESOLUTION, ".csv")
if (!file.exists(data_file)) {
  stop(paste("Data file not found:", data_file))
}

spoints.19 <- read.csv(data_file, header = TRUE, fileEncoding = "UTF-8-BOM")

# Remove spatial coordinate columns (not used in modeling)
spatial_cols <- c("i", "j", "Cell_address", "lon_c", "lat_c")
spoints.19 <- subset(spoints.19, select = -which(names(spoints.19) %in% spatial_cols))
...
cat("Data shape after cleaning:", dim(spoints.19), "\n")

# Convert categorical variables to factors
categorical_cols <- c("grid_code", "geo", "wl", "sol", "lc", "ia", "fcp")
spoints.19[categorical_cols] <- lapply(spoints.19[categorical_cols], factor)
...
spoints.19.train <- spoints.19 %>% sample_frac(TRAIN_FRACTION)
spoints.19.test <- setdiff(spoints.19, spoints.19.train)

cat("Training set size:", nrow(spoints.19.train), "\n")
cat("Testing set size:", nrow(spoints.19.test), "\n")
...
vnb.HG.19 <- train_vsurf_model(
  x = spoints.19.train[, 2:21], 
  y = spoints.19.train[, 1], 
  model_name = "HG", 
  n_vars = 20
)
...
vnb.HGPT.19 <- train_vsurf_model(
  x = spoints.19.train[, 2:23], 
  y = spoints.19.train[, 1], 
  model_name = "HGPT", 
  n_vars = 22
)
...
vnb.HG8M.19 <- train_vsurf_model(
  x = spoints.19.train[, 2:29], 
  y = spoints.19.train[, 1], 
  model_name = "HG8M", 
  n_vars = 28
)
...
print_variable_selection(vnb.HG.19, "HG")
print_variable_selection(vnb.HGPT.19, "HGPT")
print_variable_selection(vnb.HG8M.19, "HG8M")
...
cent.19 <- read.csv(prediction_file, header = TRUE, fileEncoding = "UTF-8-BOM")

# Remove spatial coordinate columns
spatial_cols <- c("i", "j", "Cell_address")
cent.19 <- subset(cent.19, select = -which(names(cent.19) %in% spatial_cols))

# Remove rows with no-data values
cent.19 <- cent.19 %>% filter_all(all_vars(!grepl(-32767, .)))
cent.19 <- cent.19[complete.cases(cent.19), ]

cat("Prediction data shape:", dim(cent.19), "\n")

# Convert categorical variables to factors
categorical_cols <- c("geo", "wl", "sol", "lc", "ia", "fcp")
cent.19[categorical_cols] <- lapply(cent.19[categorical_cols], factor)

# Make predictions with each model
tryCatch({
  cent.19$HG <- predict(vnb.HG.19, newdata = cent.19[, 2:21], step = c("pred"))
  cent.19$HGPT <- predict(vnb.HGPT.19, newdata = cent.19[, 2:23], step = c("pred"))
  cent.19$HG8M <- predict(vnb.HG8M.19, newdata = cent.19[, 2:29], step = c("pred"))
  ...
  write.csv(pred.result.19, output_file, row.names = FALSE)
