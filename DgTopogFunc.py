...
def TRI_calcu(coords,res,elev_df):
...
def TPI_calcu(coords,res,elev_df):
...
def TRI_df(dataframe,res,elev_df):
    dataframe['tri'] = np.nan
    dataframe['tri'] = [TRI_calcu(ij,res,elev_df) for ij in dataframe.index.values]
    return dataframe

def TPI_df(dataframe,res,elev_df):
